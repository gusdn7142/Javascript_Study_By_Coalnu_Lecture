...
//const API_KEY = '~~~';   //변경 필요.
let newsList = [];
let url = new URL(`http://times-node-env.eba-appvq3ef.ap-northeast-2.elasticbeanstalk.com/top-headlines?country=kr`);   //API URL Default 값 설정
...
const getLatestNews = async () => {
    url = new URL(`http://times-node-env.eba-appvq3ef.ap-northeast-2.elasticbeanstalk.com/top-headlines?country=kr`);  
    //console.log("url : ", url);
...
const getNewsByCategory = async (event) => {
    ...
    if(category === 'all') {             //category가 'all'이면
        ...
        url = new URL(
            `http://times-node-env.eba-appvq3ef.ap-northeast-2.elasticbeanstalk.com/top-headlines?country=kr&category=${category}`
        );
    }
...
const getNewsByKeyword = async () => {
    ...
    //3-2-1) 카테고리별 뉴스 검색
        if(category === 'all') {             //category가 'all'이면
            url = new URL(`http://times-node-env.eba-appvq3ef.ap-northeast-2.elasticbeanstalk.com/top-headlines?country=kr&q=${keyword}`);                
        } else {                             //category가 'Business'.....'Technology'이면                                
            url = new URL(`http://times-node-env.eba-appvq3ef.ap-northeast-2.elasticbeanstalk.com/top-headlines?country=kr&q=${keyword}&category=${category}`);          
        }
